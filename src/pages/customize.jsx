...
export default function Customize({ token }) {
  ...
  useEffect(() => {
    axios.get(`${Serv_URL}/profile/${param.undefined}`).then((response) => {
        ...
        <div className="leftbar w-screen h-screen bg-black-50 pb-5 items-center md:rounded-xl md:w-1/4 md:mt-5 md:ml-5 md:h-5/6 md:pb-0">
          <div className="flex md:flex-col lg:justify-center lg:items-center">
            <div className="w-full flex flex-col items-center">
              ...
              <button
                onClick={handleModal}
                className="text-white-50 h-10 w-24 mb-5 rounded-3xl bg-purple-50"
              >
                Change Pic
              </button>
            </div>
          ...
          <form
            ...
            className="flex flex-col mx-5 lg:justify-start items-center h-2/3 lg:h-1/2"
          >
            <textarea
              onChange={(e) => setProf_name(e.target.value)}
              value={prof_name}
              className="w-full h-1/4 md:mt-2 md:mb-1 text-center overflow-y-scroll scrollbar-thin text-gray-50 bg-black-50 border border-white-50 rounded  resize-none text-2xl py-2"
            ></textarea>
            <textarea
              onChange={(e) => setDescript(e.target.value)}
              value={descript}
              name="descript"
              className=" h-1/4 border border-white-50 my-2 rounded-2xl w-full text-center overflow-y-scroll scrollbar-thin text-gray-50 resize-none py-4 bg-black-50"
            ></textarea>
            <div className="">
              <button
                type="submit"
        ...
        <div className="right_window w-full h-full md:my-5 md:mx-5 md:w-11/12 md:h-1/2">
          <div className="flex justify-around bg-black-50 text-white-50 border border-gray-50 md:rounded-t-xl">
            <button
              ...
              onClick={renderCustomBio}
            >
              Bio
            </button>
            <button
              className="focus:bg-purple-50 border-r border-r-white-50 w-1/3"
              onClick={renderCustomTracks}
            >
              Tracks
            </button>
            <button
              className="focus:bg-purple-50 w-1/3"
              onClick={renderCustomGear}
            >
              Gear
            </button>
          </div>
          {infoDisplay}
          <div className="bg-gray-50 md:pl-5 flex md:mb-5 pb-5 pl-2 md:justify-end md:py-5 md:rounded-b-xl">
            <Link to={`/profile/${user.id}`}>
              <button className="h-10 text-white-50 px-2 rounded-3xl mr-5 border border-white-50 bg-gray-50 ">
                Return to Profile
              </button>
            </Link>
          </div>
